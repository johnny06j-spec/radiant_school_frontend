...
const TeacherOverview = ({ profile, isExecutive, onSelectTab }) => {
  const isPrimary = profile?.schoolSection === 'PRIMARY';
  const isCT = isPrimary ? true : Boolean(profile?.isClassTeacher || profile?.classTeacherOf);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
        ...
        <div style={{ backgroundColor: '#1e293b', padding: '20px', borderRadius: '12px', border: '1px solid #334155' }}>
          ...
          <p style={{ margin: 0, fontSize: '18px', fontWeight: '900', color: '#38bdf8' }}>{activeCampus}</p>
        ...
        <div style={{ backgroundColor: '#1e293b', padding: '20px', borderRadius: '12px', border: '1px solid #334155' }}>
          ...
          <p style={{ margin: 0, fontSize: '22px', fontWeight: '900', color: '#fff' }}>{profile?.schoolSection || 'PRIMARY'}</p>
        ...
        <div style={{ backgroundColor: '#1e293b', padding: '20px', borderRadius: '12px', border: '1px solid #334155' }}>
          ...
          <p style={{ margin: 0, fontSize: '22px', fontWeight: '900', color: '#d8b4fe' }}>
        ...
        <div style={{ backgroundColor: '#1e293b', padding: '20px', borderRadius: '12px', border: '1px solid #334155' }}>
          ...
          <p style={{ margin: 0, fontSize: '22px', fontWeight: '900', color: '#34d399' }}>{isCT ? ctClass : 'Subject Only'}</p>
      ...
      <div style={{ backgroundColor: '#1e293b', padding: '24px', borderRadius: '12px', border: '1px solid #334155' }}>
        <h3 style={{ margin: '0 0 16px 0', fontSize: '15px', fontWeight: 'bold', color: '#fff' }}>Quick Portal Operations</h3>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: '16px' }}>
          ...
          <button 
            onClick={() => {
              if (!isCT && !isExecutive) {
                alert("Only Class Teachers can open daily attendance registers.");
                return;
              }
              onSelectTab('ATTENDANCE');
            }}
            style={{ padding: '16px', borderRadius: '10px', backgroundColor: '#0f172a', border: '1px solid #334155', textAlign: 'left', cursor: 'pointer', transition: 'all 0.2s', opacity: (!isCT && !isExecutive) ? 0.6 : 1 }}
          >
